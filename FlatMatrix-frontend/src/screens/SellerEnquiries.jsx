...
export function SellerEnquiries() {
  ...
  const [isSolving, setIsSolving] = useState(false); // State to disable button

  useEffect(() => {
    fetchEnquiries(showSolved);
  }, [showSolved]);

  const fetchEnquiries = async (solvedStatus) => {
    const response = await getSellerEnquiries(solvedStatus);
  ...
  const handleSolve = async (enquiryId) => {
    ...
    setIsSolving(true); // Disable button
    const response = await solveEnquiry(enquiryId, reply);

    if (response.status === 200) {
      alert("Enquiry solved!");
      setReply("");
      fetchEnquiries(showSolved); // Refresh the list
    } else {
      alert(response.error.message);
    }

    setIsSolving(false); // Re-enable button
  };
            ...
            {enquiries.map((enquiry) => (
              <li key={enquiry.id} className="wishlist-item">
                <div className="wishlist-item-details2">
                  ...
                  {enquiry.status !== "SOLVED" && !showSolved && (
                    <>
                      <textarea
